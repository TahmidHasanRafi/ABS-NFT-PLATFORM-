...
def prepare_nft_metadata(image_file, name, description, wallet_address):
    try:
        prefix = wallet_address.lower()
        
        # Upload image to IPFS
        image_url, _ = upload_file_to_filebase(
            file_obj=image_file,
            key_prefix=prefix,
            filename=image_file.filename
        )

        # Build metadata (FIXED: Added comma after created_at)
        metadata = {
            "name": name,
            "description": description,
            "image": image_url,
            "creator": wallet_address,
            "created_at": int(time.time()),  # Comma added here
